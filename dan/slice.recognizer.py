...
def detect_balls_in_image(img):
    ...
    results = model.predict(source=img, conf=YOLO_CONF, iou=YOLO_IOU, classes=[SPORTS_BALL_CLASS_ID], imgsz=YOLO_IMGZ, max_det=YOLO_MAXD, verbose=False)
...
def detect_pocket_candidates(img):
    ...
    circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, dp=1.2, minDist=int(0.25 * min(h, w)), param1=100, param2=20, minRadius=rmin, maxRadius=rmax)
...
def pick_nearest(candidates, target_xy):
    ...
    return min(candidates, key=lambda c: (c[0] - target_xy[0])**2 + (c[1] - target_xy[1])**2)

def remove_balls_in_pockets(centers, radii, boxes, pockets_list, factor=POCKET_INCLUSION_FACTOR):
    kept_idx = []
    for i, (cx, cy) in enumerate(centers):
        if not any(np.hypot(cx - px, cy - py) <= pr * factor for (px, py, pr) in pockets_list):
            kept_idx.append(i)
    
    # Return filtered lists
    return [boxes[i] for i in kept_idx], [centers[i] for i in kept_idx], [radii[i] for i in kept_idx]
...
def classify_white_black_lab(img_bgr, centers, radii):
    ...
    for i, (center, r) in enumerate(zip(centers, radii)):
        ...
        cv2.circle(mask, (cx, cy), int(r * 0.8), 255, -1) # Use 80% of radius to avoid edges
...
def save_annotated(img, boxes, types, origin_pocket, out_path):
    ...
    for i, box in enumerate(boxes):
        ...
        t = types[i]
        color_map = {"white": (255, 255, 0), "black": (255, 0, 255), "other": (0, 255, 0)}
    ...
    if origin_pocket is not None:
        ...
        cv2.putText(ann, "Origin", (int(px) + 4, int(py) - 4), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 165, 255), 2)
...
def main():
    ...
    if pockets_cand:
        boxes, centers, radii = remove_balls_in_pockets(centers, radii, boxes, pockets_cand)
    ...
    if w_idx is not None: print(f"  - White ball identified at index {w_idx}")
    if b_idx is not None: print(f"  - Black ball identified at index {b_idx}")

    # --- PREPARE AND SAVE OUTPUT ---
    bl_guess = pick_nearest(pockets_cand, (0.0 * W, 1.0 * H))
    blx, bly, blr = map(float, bl_guess) if bl_guess else (0.0, float(H - 1), 0.03 * min(W, H))

    balls_json = [{"index": i, "type": t, "x_px": float(cx - blx), "y_px": float(bly - cy)} for i, ((cx, cy), t) in enumerate(zip(centers, types))]
