...
def full_analyzer_pipeline(image_path: str) -> AnalyzerResult:
    ...
    if candidates:
        ...
        if len(contenders) == 1:
            ...
            Debugger.log(
                f"⚔️ Multi-Tie detected! {len(contenders)} candidates within 10 points range."
            )
            ...
            for ball in contenders:
                ...
                if path and os.path.exists(path):
                    # בדיקת המודל
                    current_ml_score = test_is_white_model.get_white_score(
                        cv2.imread(path)
                    )

                Debugger.log(
                    f"   - Candidate at {ball.center}: Legacy={_white_avg(ball):.1f} | ML Conf={current_ml_score:.4f}"
                )
            ...
            Debugger.log(
                f"   🏆 ML Chose ball at {best_ml_ball.center} with conf {best_ml_score:.4f}"
            )
