...
def analyze_ball_brightness(
    ...
    for i, ball in enumerate(balls, 1):
        ...
        ball_path = get_output_path(f"ball_{i}_({cx}_{cy})_r{r}.png", sub_dir="balls")
...
if __name__ == "__main__":
    ...
    if not sub_photos or not main_photo:
        ...
        for i, b in enumerate(sorted_balls, 1):
            print(
                f"⚪ Ball #{i}: center=({b.center[0]}, {b.center[1]}), radius={b.radius}"
            )
        ...
        whitest, darkest = analyze_ball_brightness(
            full_path, sorted_balls, os.path.join(base_dir, "out", "balls")
        )
        print(
            f"\n✨ Whitest ball:  center={whitest.center if whitest else None}, radius={whitest.radius if whitest else None}"
        )
        print(
            f"⚫ Darkest ball:  center={darkest.center if darkest else None}, radius={darkest.radius if darkest else None}"
        )
