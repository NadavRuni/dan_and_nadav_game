...
def find_table_rectangle(image_or_path: Union[str, np.ndarray]) -> Optional[Rectangle]:
    ...
    lines = cv2.HoughLinesP(
        edges, rho=1, theta=np.pi / 180, threshold=120, minLineLength=100, maxLineGap=30
    )
    if lines is None:
    ...
    Debugger.log(
        f"✅ Rectangle edges found (OpenCV): left={left_x}, right={right_x}, top={top_y}, bottom={bottom_y}"
    )
    return Rectangle(
